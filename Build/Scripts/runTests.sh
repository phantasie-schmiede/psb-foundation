...
read -r -d '' HELP <<EOF
psb_foundation test runner. Execute unit test suite and some other details.
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="public/typo3conf/ext/psb_foundation/${1}"
else
    case ${TEST_SUITE} in
        functional)
            TEST_FILE="public/typo3conf/ext/psb_foundation/Tests/Functional"
            ;;
        unit)
            TEST_FILE="public/typo3conf/ext/psb_foundation/Tests/Unit"
            ;;
    esac
fi
